...
func (h *userHandlers) Add(params user.AddUserParams, principal interface{}) middleware.Responder {
	...
	userData := params.Body

	// Если нет имени, то возвращаем код 400: Invalid input
	if userData.FirstName == "" {
		return user.NewAddUserBadRequest()
	}

	// вызываем метод из контроллера, т.к. мы не можем напрямую работать с репозиторием
	//*********************************************************************************
	result, err := h.controller.AddUser(userData)
